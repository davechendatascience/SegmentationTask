"""
Train DINOv2 query-based instance segmentation.
...
Usage:
    python -m scripts.dinov2_seg.train
"""
import argparse
import contextlib
import math
...
def train(data_cfg=None, model_cfg=None, train_cfg=None, aug_cfg=None, resume=None):
    ...
    model = DINOv2SegModel.build(
        model_name=model_cfg.dinov2_model,
    ...
    # Optimizer
    proj_lr = train_cfg.learning_rate * train_cfg.backbone_proj_lr_factor
    param_groups = model.get_param_groups(proj_lr=proj_lr, decoder_lr=train_cfg.learning_rate)
    ...
    for epoch in range(start_epoch, train_cfg.num_epochs):
        model.train()
        model.backbone.vit.eval()  # keep frozen DINOv2 in eval mode
